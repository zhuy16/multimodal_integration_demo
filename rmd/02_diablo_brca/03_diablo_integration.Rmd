...
bind_rows(
  single_omics_acc,
  data.frame(block = "DIABLO (weighted vote)", overall_acc = overall_acc)
) |>
  ggplot(aes(x = reorder(block, overall_acc), y = overall_acc,
             fill = block == "DIABLO (weighted vote)")) +
  ...
  labs(
    ...
    subtitle = "DIABLO (red) should outperform all individual blocks",
...
| **DIABLO (all 3)** | **`r if(exists('cv_ber')) sprintf("%.1f%%", 100*cv_ber) else "—"`** | **`r sprintf("%.1f%%", 100*overall_acc)`** |

**Key conclusion**: Multi-omics integration with DIABLO outperforms every single-omics baseline,
demonstrating that mRNA, miRNA, and proteomics carry **complementary, non-redundant** information
about PAM50 subtype identity.
